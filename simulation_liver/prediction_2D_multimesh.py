...
class AnimationStepController(Sofa.Core.Controller):
    ...
    def createGraph(self, rootNode):
        ...
        self.MO1_LR = self.mapping.addObject('MechanicalObject', name='DOFs_HR', template='Vec3d', src='@../../SamplingNodes/coarseGridHigh')
        ...
        for nb_nodes, mesh in self.meshes.items():
            ...
            self.trained_MOs.append(self.child_nodes[-1].CoarseMesh.addObject('MechanicalObject', name='DOFs_HR', template='Vec3d', src='@../../SamplingNodes/coarseGridHigh'))
            self.child_nodes[-1].CoarseMesh.addObject('SphereCollisionModel', radius=sphereRadius, group=1, color='0 1 0')
            self.child_nodes[-1].CoarseMesh.addObject('BarycentricMapping', name="mapping", input='@../DOFs', input_topology='@../triangleTopo', output='@DOFs_HR')
        ...
        self.MO_training = self.mapping.addObject('MechanicalObject', name='DOFs_HR', template='Vec3d', src='@../../SamplingNodes/coarseGridHigh')
    ...
    def onSimulationInitDoneEvent(self, event):
        ...
        self.high_res_shape = self.MO1_LR.position.value.shape
        self.low_res_shape = self.MO_training.position.value.shape
...
def main():
    ...
    meshes = [f for f in os.listdir('mesh') if 'liver' in f and f.endswith('.msh')]
