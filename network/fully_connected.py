...
class Data(Dataset):
    ...
    def __init__(self, data_dir):
        ...
        try:
            self.data_3D = np.load(f'{self.data_dir}/CoarseResPoints.npy')
        except FileNotFoundError:
            self.data_3D = np.load(f'{self.data_dir}/CoarseResPoints_normalized.npy')
            self.normalized = True
        self.data = self.data_3D.reshape(self.data_3D.shape[0], -1)
        self.input_size = self.data.shape[1]
        try:
            self.labels_3D = np.load(f'{self.data_dir}/HighResPoints.npy')
        except FileNotFoundError:
            self.labels_3D = np.load(f'{self.data_dir}/HighResPoints_normalized.npy')
...
if __name__ == "__main__":
    train = True
    if train:
        data_dir = 'npy/2024-05-23_16:39:07/train'
