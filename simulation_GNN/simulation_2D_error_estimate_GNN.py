...
class AnimationStepController(Sofa.Core.Controller):
    ...
    def createGraph(self, rootNode):
        ...
        self.coarseGrid = self.coarse.addObject('RegularGridTopology', name='coarseGrid', min=p_grid_LR.min, max=p_grid_LR.max, nx=p_grid_LR.res[0], ny=p_grid_LR.res[1], nz=p_grid_LR.res[2])
        self.coarseTopo = self.coarse.addObject('TriangleSetTopologyContainer', name='triangleTopo', src='@coarseGrid')
    ...
    def onAnimateEndEvent(self, event):
        ...
        edges_high = self.compute_edges(self.coarseTopo)
        ...
        if self.save and not self.efficient_sampling:    
            ...
            np.save(f'npy_GNN/{self.directory}/HighResPoints_{round(self.magnitudes[self.count_m], 3)}_x_{round(self.versors[self.count_v][0], 3)}_y_{round(self.versors[self.count_v][1], 3)}.npy', np.array(U_high))
            np.save(f'npy_GNN/{self.directory}/CoarseResPoints_{round(self.magnitudes[self.count_m], 3)}_x_{round(self.versors[self.count_v][0], 3)}_y_{round(self.versors[self.count_v][1], 3)}.npy', np.array(U_low))
            np.save(f'npy_GNN/{self.directory}/EdgesHigh_{round(self.magnitudes[self.count_m], 3)}_x_{round(self.versors[self.count_v][0], 3)}_y_{round(self.versors[self.count_v][1], 3)}.npy', np.array(edges_high))
            np.save(f'npy_GNN/{self.directory}/EdgesLow_{round(self.magnitudes[self.count_m], 3)}_x_{round(self.versors[self.count_v][0], 3)}_y_{round(self.versors[self.count_v][1], 3)}.npy', np.array(edges_low))
        else:
