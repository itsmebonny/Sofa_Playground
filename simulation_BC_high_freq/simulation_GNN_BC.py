...
from parameters_2D_zigzag import p_grid, p_grid_LR
...
class AnimationStepController(Sofa.Core.Controller):
    ...
    def createGraph(self, rootNode):
        ...
        rootNode.addObject('VisualStyle', name="visualStyle", displayFlags="showBehaviorModels showCollisionModels showForceFields")
        ...
        self.exactSolution.addObject('MeshGmshLoader', name='grid', filename='mesh/zigzag_4445.msh')
        ...
        self.exactSolution.addObject('StaticSolver', name='ODE', newton_iterations="20", printLog=False)
        self.exactSolution.addObject('CGLinearSolver', iterations=4500, name="linear solver", tolerance="1.0e-6", threshold="1.0e-6") 
        ...
        self.LowResSolution.addObject('MeshGmshLoader', name='grid', filename='mesh/zigzag_896.msh')
        ...
        self.LowResSolution.addObject('StaticSolver', name='ODE', newton_iterations="20", printLog=False)
        self.LowResSolution.addObject('CGLinearSolver', iterations=5000, name="linear solver", tolerance="1.0e-6", threshold="1.0e-6")
    ...
    def onSimulationInitDoneEvent(self, event):
        ...
        if self.save:
            if not os.path.exists('npy_GNN_hf'):
                os.mkdir('npy_GNN_hf')
            ...
            os.makedirs(f'npy_GNN_hf/{self.directory}')
            print(f"Saving data to npy_GNN_hf/{self.directory}")
        self.sampled = False
    ...
    def onAnimateBeginEvent(self, event):
        ...
        if not self.efficient_sampling:
            ...
            self.versor = np.append(self.versor, 0)
            self.magnitude = np.random.uniform(0, 30)
       ...
       # Define random box
        ...
        if side == 2:
            ...
            y_min = 0.29
            y_max = 1.01
        # Apply decay to magnitude based on x position
        ...
        max_magnitude = compute_force(x_min, self.versor, 40)
        self.magnitude = np.random.uniform(1, max_magnitude)
        ...
        self.cff = self.exactSolution.addObject('ConstantForceField', indices=indices, totalForce=self.externalForce, showArrowSize=0.3, showColor="0.2 0.2 0.8 1")
        self.cff.init()
    ...
    def onAnimateEndEvent(self, event):
        ...
        if self.save and self.bad_sample == False:
            self.tmp_dir = f'npy_GNN_hf/{self.directory}/sample_{self.iteration}'
